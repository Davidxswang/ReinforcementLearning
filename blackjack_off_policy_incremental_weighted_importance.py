...
def mc_control_off_policy_incremental_weighted(env, gamma, n_episode, behavior_policy):
    """Obtain the optimal policy with off-policy Monte Carlo control with incremental way of updating the Q function and weighted importance sampling
    Args:
        env: OpenAI Gym environment
    ...
    for episode in range(n_episode):
        ...
        for state_t, action_t, reward_t in zip(states_t[::-1], actions_t[::-1], rewards_t[::-1]):
            ...
            number[(state_t, action_t)] += weight
...
optimal_Q, optimal_policy = mc_control_off_policy_incremental_weighted(env, gamma, n_episode, random_policy)
